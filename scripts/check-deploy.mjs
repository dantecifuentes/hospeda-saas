...
const errors=[];
const required=['DATABASE_URL','JWT_SECRET','CLIENT_ORIGINS','PUBLIC_API_URL','PUBLIC_WEB_URL','VITE_API_BASE_URL','STORAGE_DIR'];
for(const name of required)if(!process.env[name])errors.push(`${name}: falta configurar`);
if(process.env.STORAGE_DIR&&(!process.env.STORAGE_DIR.startsWith('/')||process.env.STORAGE_DIR==='/'))errors.push('STORAGE_DIR: requiere una ruta absoluta de volumen persistente');
